...
def update_dictionary_paths(args):
    ...
    if args.dataset_name_or_path in ["AAP", "cometa_knn_clinical", "ispo"]:
        args.train_dir = f"./data/{args.dataset_name_or_path}/train.txt"
        args.eval_dir = f"./data/{args.dataset_name_or_path}/test.txt"
    else:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=['ncbi-disease', 'bc5cdr-chemical', 'bc5cdr-disease', 'AAP','cometa_knn_clinical', 'ispo']
    )
    ...
    # 我们的实验是根据epoch进行迭代训练，如果想要得到更好的结果，可以调整降低step_ratio，参考交叉编码器便可很简单的实现
    ...
    parser.add_argument('--use_embed_parallel', default=True, type=bool)
